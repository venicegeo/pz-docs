...
In order to use this you can look up the EventTypeId for the system EventType by hitting the `/eventType?name=piazza:ingest` endpoint (or whichever EventType name you want returned).  You can get the eventTypeId from the EventType that is returned.  Then you can create your Trigger with that EventTypeId.
