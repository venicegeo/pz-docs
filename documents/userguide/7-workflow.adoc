...
The Workflow provides some system level EventTypes: `piazza:ingest` and `piazza:executionComplete`.  These two system EventTypes can be used to set up Triggers when something is ingested or an execution has finished. Those two EventTypes look like the following:
...
An Event of type `piazza:ingest` will be automatically POSTed by the system after a file or image is ingested Piazza's Loader mechanism.

An Event of type `piazza:executionComplete` will be automatically POSTed by the system when the User Service Registry completes an execution of a service.

In order to use this you can look up the EventTypeId for the system EventType by hitting the `/eventType?name=piazza:ingest` endpoint (or whichever EventType name you want returned).  The return data value will be an array with a single EventType in it. You can get the eventTypeId from the EventType that is returned.  Then you can create your Trigger with that EventTypeId.
