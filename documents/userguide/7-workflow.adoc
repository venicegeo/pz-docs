...
The user may generate an _Event_ of that _EventType_ to indicate some interesting condition has occurred. The _Event_ object is POSTed to `/event` and contains the ID of the _EventType_, and the parameters of the event. For example:
...
Lastly, cronSchedule can be specified using the `@every duration` notation, where duration is replaced by a Golang parsable time.Duration. Examples include:
